...
- **Go toolchain 1.26+** on `PATH` (`go version`).
- **Network + public proxy** reachable so `go mod tidy` can download `go-lib` (`go env GOPROXY` — the default `https://proxy.golang.org,direct` is fine; fail if offline / `GOPROXY=off`).
- **`mcp__go-lsp__go_diagnose`** registered for post-write diagnostics.
...
| Item | Default |
|------|---------|
| Git module path prefix | `github.com/labspangaea` — use silently |
...
{
  "Name":          "order-service",
  "Module":        "github.com/labspangaea/order-service",
  "Entity":        "Order",
  "EntityLower":   "order",
  "ApperrBase":    1000,
  "Type":          "api",
...
Generate `{name}/` as a **sibling directory to `go-lib`** (not inside it). If the directory already exists, **MUST ask before overwriting**.
...
The same recipes are also rendered by `/go-scaffolder:create-go-repository` Step 7 for the single-repository case; keep the reference file in sync if either changes.
...
| `{{.Module}}` | module prefix + `name` | `github.com/labspangaea/order-service` |
...
Local style + error-wrapping rules baked into the templates live at `${CLAUDE_SKILL_DIR}/references/code-style.md`. Read it before authoring or hand-tweaking generated Go files. The wider cross-cutting Go rulebook is in `labspangaea-code-standard/references/go.md`.
