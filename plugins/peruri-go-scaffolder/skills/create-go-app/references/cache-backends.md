...
`/peruri-go-scaffolder:create-go-repository` Step 7 points readers here for the same wiring snippets when adding a single repository to an existing project. The `main_api_*` template's `NewCached` wiring renders the canonical form; this file is the human-readable index.
