...
End-to-end runtime verification for services scaffolded by `peruri-go-scaffolder`. The heavy lifting lives in the bundled bash driver at `${CLAUDE_SKILL_DIR}/scripts/run.sh` — your job here is to do a friendly pre-flight, invoke the script, and translate its output for the user.
...
```bash
LIB="$HOME/go/src/sipgn-git.bgn.go.id/harry.sitohang/go-peruri-lib"
COMPOSE="$HOME/projects/claude-plugins/plugins/peruri-go-scaffolder/docker-compose.yml"
...
if ! grep -q 'OffsetParamsFromRequest' "$LIB/db/repo/offset.go" 2>/dev/null; then
  echo "MISSING: go-peruri-lib at $LIB does not have OffsetParamsFromRequest."
  echo "Run: cd \"$LIB\" && git pull origin master"
  exit 2
fi
...
If any check fails, surface the exact line to the user — most of these are one-command fixes (install jq, pull go-peruri-lib, etc.). Do not try to install or pull on the user's behalf; that's their decision.
...
```bash
docker compose -f "$HOME/projects/claude-plugins/plugins/peruri-go-scaffolder/docker-compose.yml" down -v
```
