# codebase.md — @peruri/ts-lib API Reference

Source: <https://sipgn-git.bgn.go.id/harry.sitohang/peruri-ts-lib>

TypeScript/Bun port of `go-peruri-lib`. Every package is importable via a subpath
(`@peruri/ts-lib/<pkg>`) or from the root barrel (`@peruri/ts-lib`). Durations are
...
## `@peruri/ts-lib/pubsub` (+ `/kafka` `/rabbitmq` `/redis`)
...
newKafkaPublisher(kafka: Kafka, ...opts) / newKafkaConsumer(kafka, ...opts)        // '@peruri/ts-lib/pubsub/kafka'
