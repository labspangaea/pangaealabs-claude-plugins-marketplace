...
Scaffold a production-ready ElysiaJS application on Bun that wires `@peruri/ts-lib`
(the TypeScript port of `go-peruri-lib`).
...
- **`@peruri/ts-lib` resolvable.** The source is read from the config store
  (`~/.peruri-elysia-scaffolder/config.json` → `tsLibSource` / `tsLibDependency`, env-overridable,
  defaults to the BGN GitLab repo + `file:../peruri-ts-lib` — see `references/config-store.md`).
  Either (a) a Bun **workspace** sibling checkout exists (the `replace ../lib` analog, preferred for
  local dev; cloned from `tsLibSource` if missing), or (b) the **GitLab private npm registry** is
  configured in `~/.bunfig.toml` / `.npmrc` for the `@peruri` scope with a valid token (the GOPRIVATE
  analog). Best-effort sync; never block scaffolding on a registry hiccup — fall back to workspace.
...
| Item | Default |
|------|---------|
| npm scope prefix | `@peruri` — use silently for the app's own package name unless told otherwise |
...
Set `TsLibDependency` to the `TS_LIB_DEPENDENCY` value resolved in preflight (config store
`~/.peruri-elysia-scaffolder/config.json`, default `file:../peruri-ts-lib` — see
`references/config-store.md`); `package.json.tmpl` writes it verbatim.
...
Generate `{name}/` as a **member of the Bun workspace** (sibling to `packages/peruri-ts-lib`, under
`examples/` or a path the user names). If the directory already exists, **MUST ask before overwriting**.
...
- **HTTP error dispatch** (MUST): `throw new ApiError(...)` from the controller; the Elysia `onError`
  hook (provided by `@peruri/ts-lib/server`) renders the envelope and sets the status. Never build error
  JSON by hand.
...
| `{{Module}}` | `@peruri/` + `name` | `@peruri/order-service` |
...
```bash
bun install                       # resolves @peruri/ts-lib (workspace or registry)
...
- **Telemetry (env-driven OTLP/HTTP traces+logs via `@peruri/ts-lib/telemetry`):** `${CLAUDE_SKILL_DIR}/references/telemetry.md`
