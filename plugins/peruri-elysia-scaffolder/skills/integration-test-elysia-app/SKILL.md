...
End-to-end runtime verification for services scaffolded by `peruri-elysia-scaffolder`.
...
```bash
PERURI_TS_LIB="$LIB" ${CLAUDE_SKILL_DIR}/scripts/run.sh
```

The script handles everything: render → `docker compose up -d --wait` → `bun install` →
`tsc --noEmit` → seed → boot on `:8099` → assertions → SIGTERM. The generated package's
`@peruri/ts-lib` dep is repointed at the absolute `$LIB` checkout (the template default
`file:../peruri-ts-lib` assumes a generated sibling; the test out-dir is elsewhere).
