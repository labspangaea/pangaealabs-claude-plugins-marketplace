...
Shared D2 diagrams are consumed from a manifest produced by render_diagrams.py
(PDF for handbook, SVG for marp), so a diagram looks identical across templates.
"""
...
def build_handbook(args, tmpl_dir: Path, manifest: dict, merged: dict,
    ...
    lua = [SCRIPTS / "diagram.lua"] + [assets / f for f in manifest.get("lua_filters", [])]
    ...
    env = dict(os.environ)
    if args.diagrams_manifest:
        env["DOCSMITH_DIAG_MANIFEST"] = str(Path(args.diagrams_manifest).resolve())
    r = subprocess.run(cmd, env=env)
    return r.returncode


def build_marp(args, tmpl_dir: Path, manifest: dict, merged: dict,
               profile: dict, src: Path, out: Path) -> int:
    ...
    slides_md, _ = marp_prep.prepare(src.read_text(), args.diagrams_manifest,
                                     theme_name, footer=footer)
...
def main() -> int:
    ...
    if backend == "pandoc-tectonic":
        rc = build_handbook(args, tmpl_dir, manifest, merged, profile, src, out)
    elif backend == "marp-cli":
        rc = build_marp(args, tmpl_dir, manifest, merged, profile, src, out)
    else:
