#!/usr/bin/env bun
/**
 * smoke — compile-only matrix runner for elysia-scaffolder (maintainer tool).
 *
 ...
 * compile against @labspangaea/ts-lib across the parameter matrix. It does NOT run the
...
// @labspangaea/ts-lib is a public npm package, so smoke type-checks against the
// version the generated package.json pins (^0.1.0), fetched from the public
// registry like any other dependency — no local checkout, no config store.
const TS_LIB_VERSION = '^0.1.0';
...
function params(c: Combo): string {
  return JSON.stringify({
    Name: c.id, Module: `@labspangaea/${c.id}`, Entity: 'Order', EntityLower: 'order',
...
async function ensureDeps(): Promise<string> {
  ...
  await writeFile(join(deps, 'package.json'), JSON.stringify({
    ...
    dependencies: {
      '@labspangaea/ts-lib': TS_LIB_VERSION,
